...
HOMAOB <- as.character(HOMAOB)
HOMAOB[ which( HOMAOB == "IR_Sobrepeso_T2" )] <- "IR_Obeso_T2"
HOMAOB[ which( HOMAOB == "NoIR_Sobrepeso_T2" )] <- "NoIR_Obeso_T2"

HOMAOB[ which( HOMAOB == "NoIR_Normopeso_T2" )] <- "NoIRNormopeso"
HOMAOB[ which( HOMAOB == "NoIR_Obeso_T2" )] <- "NoIRSobOb"
HOMAOB[ which( HOMAOB == "IR_Obeso_T2" )] <-"IRSobOb"

HOMAOB <- factor(HOMAOB)

HOMAOB <- factor(HOMAOB,levels(HOMAOB)[c(2,3,1)])
...
colnames(factored.design)[1:3] <- c("NoIRNormopeso","NoIRSobOb","IRSobOb")
 colnames(factored.design)[4:14]  <- c("CD8T","CD4T","NK","BCELL","MONO","NEU","SEX","AGE","BATCH","ORIGEN1","ORIGEN2")
...
#Now we can make any comparisons between the experimental conditions in the usual way, example:
cm <- makeContrasts(NoIRNormopesoVSIRSobOb=NoIRNormopeso-IRSobOb,NoIRSobObVSIRSobOb=NoIRSobOb-IRSobOb, levels=factored.design)
#Then compute these contrasts and moderated t-tests:
fit2 <- contrasts.fit(data.fit, cm)
fit2 <- eBayes(fit2)
data.fit.eb <- fit2
...
write.csv2(DMPs_significants, file="/home/augusto/Descargas/RESUMEN_RESULTS_EPIGENETICA_2021/ANALISIS_EXTRA/NoIRNormopesoVSIRSobOb_PUBER_adjusted_LEUCOS_AGE_SEX_rawP0001.csv", row.names=FALSE)
...
write.csv2(DMPs_significants, file="/home/augusto/Descargas/RESUMEN_RESULTS_EPIGENETICA_2021/ANALISIS_EXTRA/NoIRSobObVSIRSobOb_PUBER_adjusted_LEUCOS_AGE_SEX_rawP0001.csv", row.names=FALSE)
